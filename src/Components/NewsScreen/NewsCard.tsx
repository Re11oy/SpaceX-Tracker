...
const NewsCard: React.FC<Props> = ({ data, fullWidth }) => {
  ...
  return (
    <Image resizeMode="cover" source={{ uri: featured_image }} fullWidth={fullWidth}>
      <TouchableOpacity onPress={() => Linking.openURL(url)} style={{ flex: 1 }}>
        ...
        <ContentWrapper colors={['#37346800', '#222437']} start={{ x: 0.0, y: 0.4 }} end={{ x: 0.0, y: 0.95 }}>
          <TextWrapper>
            <Title>{title}</Title>
            <NewsSite>
